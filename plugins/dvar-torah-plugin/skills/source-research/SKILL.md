...
### 3. מציאת קשרים
- `english_semantic_search` — גילוי ראשוני, טקסטים תימטיים
- `get_links_between_texts` — קישורים בין מקורות
- `get_topic_details(<topic>)` — מידע רקע על נושאים
- `search_in_dictionaries(<term>)` — הגדרות מונחים

## כללי חיפוש

1. **רוחב**: חפש בלפחות 3 מקורות שונים
2. **עומק**: אם מצאת מקור רלוונטי, חפש גם את ההקשר הרחב שלו
3. **קשרים**: בדוק קישורים בין המקורות שמצאת
